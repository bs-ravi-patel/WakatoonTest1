    ...
    class func downloadEpisodeFromURL(_ videoLink: String, isFor: APIManager.VideoLabel, loopTimecode: Double?) {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now()) {
            ...
            DownloadManager(name: UUID().uuidString, url: videoURL).download { percent in
                ...
                let savedVideoString = url.lastPathComponent
                switch isFor {
                    case .DETECTED_LOOP:
                        VideoCacheModel(savedFileName: savedVideoString).saveCacheVideo(.DETECTED_LOOP_DATA, loopTimecode: loopTimecode)
                        break
                    case .EPISODE:
                        VideoCacheModel(savedFileName: savedVideoString).saveCacheVideo(.EPISODE_DATA, loopTimecode: loopTimecode)
                        break
                    case .INTRO:
                        VideoCacheModel(savedFileName: savedVideoString).saveIntroCacheVideo(loopTimecode: loopTimecode)
                        break
                    case .DETECTED:
                        VideoCacheModel(savedFileName: savedVideoString).saveCacheVideo(.DETECTED_DATA, loopTimecode: loopTimecode)
                }
            }.onError { (error) in
